...
def classify_image(
    ...
    try:
        ...
        processed_img = preprocess_image(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        ...
        if enable_visualization:
            try:
                ...
                visualization = np.zeros((640, 640, 3), dtype=np.uint8)  # 生成灰色背景
...
def batch_process(files: List[str], model_name: str, progress: gr.Progress = gr.Progress(),
    ...
    for i, f in enumerate(files):
        ...
        try:
            ...
            data, vis = classify_image(img, model_name=model_name, system_monitor=SystemMonitor())
...
with gr.Blocks(title=CONFIG.get("title", "图像分类系统")) as app:
    ...
    # 事件绑定
    single_btn.click(
        lambda img, model, conf, vis: classify_image(img, model, conf, vis, system_monitor=system_monitor),
